...
export function Sidebar({ userName: propUserName, userPhoto: propUserPhoto }: SidebarProps) {
    ...
    const activeParentId = USER_NAV_DATA.flatMap((section) => section.items).find((item) =>
        item.items.some((subItem: UserSubItem) => subItem.url === pathname)
    )?.id;

    // Use Redux state if available, otherwise fallback to props
    const userName = user ? `${user.prenom || ''} ${user.nom || ''}`.trim() || propUserName || t('common.user') : (propUserName || t('common.user'));
                            ...
                            <div key={section.id} className="mb-6">
                                {!isCollapsedDesktop && (
                                    <h2 className="mb-4 px-3 text-sm font-bold uppercase tracking-widest text-gray-500/70">
                                        {t(`nav.user.sections.${section.id}`)}
                                    </h2>
                                )}

                                <nav role="navigation" aria-label={t(`nav.user.sections.${section.id}`)}>
                                                ...
                                                {isCollapsedDesktop ? (
                                                    ...
                                                    <div>
                                                        ...
                                                        >
                                                            <item.icon
                                                                ...
                                                                aria-hidden="true"
                                                            />
                                                            <span>{t(`nav.user.items.${item.id}`)}</span>
                                                            <ChevronUp
                                                                className={cn(
                                                    ...
                                                    >
                                                        <item.icon
                                                            ...
                                                            aria-hidden="true"
                                                        />
                                                        {!isCollapsedDesktop && <span>{t(`nav.user.items.${item.id}`)}</span>}
                                                    </MenuItem>
                                                )}
                                            </li>
                                        ))}
