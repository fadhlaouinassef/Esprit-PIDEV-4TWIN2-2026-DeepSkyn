...
// Blob class moved outside component
class Blob {
  ...
  constructor(
    ...
    this.rtScene = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({
        color: 0x000000,
        // @ts-ignore - onBeforeCompile is not in types but exists
    ...
    // @ts-ignore - defines exists on material
...
export default function InteractivePortrait() {
  ...
  useEffect(() => {
    ...
    // @ts-ignore - onBeforeCompile exists
    bgPlaneMaterial.onBeforeCompile = (shader: any) => {
    ...
    // @ts-ignore - onBeforeCompile exists
    helmetImageMaterial.onBeforeCompile = (shader: any) => {
      shader.uniforms.texBlob = { value: blob.rtOutput.texture }
      let vertexShader = shader.vertexShader
      vertexShader = vertexShader.replace("void main() {", "varying vec4 vPosProj;\nvoid main() {")
      vertexShader = vertexShader.replace(
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full h-full bg-[#1a1f1a] cursor-crosshair overflow-hidden"
