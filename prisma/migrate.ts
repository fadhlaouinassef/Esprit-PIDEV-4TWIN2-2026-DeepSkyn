...
async function migrate() {
  try {
    ...
    // Table SkinScoreAnalysis (snapshots d'analyse détaillée)
    await prisma.$executeRawUnsafe(`
      CREATE TABLE IF NOT EXISTS "SkinScoreAnalysis" (
        "id" SERIAL PRIMARY KEY,
        "user_id" INTEGER NOT NULL,
        "quiz_id" INTEGER,
        ...
        "routine" JSONB,
        "summary" TEXT,
        "trigger" TEXT DEFAULT 'progress' NOT NULL,
        "created_at" TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
        FOREIGN KEY ("user_id") REFERENCES "User"("id") ON DELETE CASCADE,
    ...
    await prisma.$executeRawUnsafe(
      'CREATE INDEX IF NOT EXISTS "SkinScoreAnalysis_user_id_created_at_idx" ON "SkinScoreAnalysis"("user_id", "created_at");'
    );
